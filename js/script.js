/* DAW FX RACK - REMASTERED LOGIC 
   Features: Advanced Visualizers, Smooth Animations, Audio Processing
*/
...
class DAWApp {
    ...
    initAudioContext() {
        ...
        this.audio.analyser.smoothingTimeConstant = 0.85; // Make analysis smoother
    ...
    initEventListeners() {
        ...
        this.dom.fileInput.addEventListener('change', async (e) => {
            if(e.target.files.length > 0) {
                await this.triggerOverlay("Importing Audio...", 1500);
                this.handleFileLoad(e);
            }
        });

        this.dom.downloadBtn.addEventListener('click', async () => {
            await this.triggerOverlay("Rendering WAV...", 2000);
            this.showToast("Export Complete (Simulated)");
        });
        ...
        // Drag Logic
        this.dom.fxChainContainer.addEventListener('dragstart', e => { e.target.classList.add('dragging'); e.dataTransfer.effectAllowed = 'move'; });
        this.dom.fxChainContainer.addEventListener('dragend', e => { e.target.classList.remove('dragging'); this.updateChainOrder(); });
    ...
    updateNodeParam(fxId, paramId, value) {
        ...
        if(paramId === 'bypass') { group.input.gain.setTargetAtTime(value ? 0 : 1, t, 0.05); return; }
...
class VisualizerEngine {
    constructor(canvas, analyser) {
        ...
        this.mode = 'bars'; // Default
        this.dataArray = new Uint8Array(analyser.frequencyBinCount);
        // Smooth buffer for lerping
        this.prevData = new Float32Array(analyser.frequencyBinCount).fill(0);
        this.resize();
    ...
    draw() {
        ...
        for (let i = 0; i < this.dataArray.length; i++) {
            this.prevData[i] += (this.dataArray[i] - this.prevData[i]) * 0.3; // 0.3 is the smoothing factor
        }
        ...
        // SWITCH MODES
        if (this.mode === 'bars') {
            this.drawBars(ctx, w, h, accent);
        } else if (this.mode === 'wave') {
            this.drawWave(ctx, w, h, accent);
        } else if (this.mode === 'circular') {
            this.drawCircular(ctx, w, h, accent);
        } else if (this.mode === 'mirror') {
            this.drawMirror(ctx, w, h, accent);
        } else if (this.mode === 'nebula') {
            this.drawNebula(ctx, w, h, accent);
        }
    }
    ...
    drawBars(ctx, w, h, color) {
        ...
        for (let i = 0; i < bufferLength; i++) {
            const barHeight = (this.prevData[i] / 255) * h;
    ...
    drawCircular(ctx, w, h, color) {
        ...
        for (let i = 0; i < bars; i++) {
            const value = this.prevData[i * step];
    ...
    drawMirror(ctx, w, h, color) {
        ...
        for (let i = 0; i < bufferLength; i++) {
            const barHeight = (this.prevData[i] / 255) * (h / 2);
            ctx.globalAlpha = 0.6;
            // Top half
            ctx.fillRect(w/2 + x, h/2 - barHeight, barWidth, barHeight);
            ctx.fillRect(w/2 - x, h/2 - barHeight, barWidth, barHeight);
            // Bottom half
            ctx.fillRect(w/2 + x, h/2, barWidth, barHeight);
            ctx.fillRect(w/2 - x, h/2, barWidth, barHeight);
